...
export class Engine {
    ...
    _updateUserFundsOrHoldings(executedQuantity: number, fills: Fill[], price: number, baseAsset: string, quoteAsset: string, side: "buy" | "sell", userId: string): void {
        ...
        if (side == "buy") {
            ...
            // transfer amount to fill.forEach -> fillOwnerId
            fills.forEach(fill => {
            ...
            /**
             * for sell:
             * - deduct quantity from userId's lockedholdings
             * - fill.forEach -> transfer that quantity to fillOwnerId's holding and deduct fill.quantity*fill.price amount from fillowner.lockedbalance
             */
            fills.forEach(fill => {
